...
def clear_chat_history():
    ...
    st.experimental_rerun() # Reload the app to update the UI

# --- New Feature: Reset Conversation ---
def reset_conversation_and_model_memory():
    """
    Clears the chat history from Redis, resets the Streamlit session messages,
    and effectively resets the Gemini model's memory by re-initializing the chat.
    """
    clear_chat_history() # Clear Redis and session state
    # The model's internal chat history is reset when `model.start_chat()` is called
    # with an empty history, which happens when st.session_state.messages is empty.
    # We just need to ensure the session state is truly empty and rerun.
...
# New Button: Reset Conversation
st.sidebar.button("Reset Conversation", on_click=reset_conversation_and_model_memory)
